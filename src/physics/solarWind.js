...
export function computeExternalB(xKm, yKm, zKm, solarWindParams) {
  ...
  const r0 = computeStandoffDistance(Dp, imfBz);
  const b0Cf = 25 * Math.pow(Dp / 2, 1 / 3); // nT base strength
  const cosThetaSun = Math.cos(thetaSun);
  // Only active on dayside (cos > 0), falls off as (r0/r)^3
  const cfStrength = b0Cf * Math.pow(r0 / Math.max(rRe, 1), 3)
    * Math.max(0, cosThetaSun) * Math.max(0, cosThetaSun);
  ...
  const b0Tail = 20 * Math.pow(Dp / 2, 0.5) * (1 + Math.abs(dst) / 100); // nT
  // Ramp on for nightside: f(x_gsm) approaches 1 for x_gsm << -5 Re
  const tailRamp = 0.5 * (1 - Math.tanh((xGsm / Re + 5) / 3));
